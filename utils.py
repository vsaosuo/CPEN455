...
def discretized_mix_logistic_loss_per_image(x, l):
    """ log-likelihood for mixture of discretized logistics, assumes the data has been rescaled to [-1,1] interval """
    # Pytorch ordering
    x = x.permute(0, 2, 3, 1)
    l = l.permute(0, 2, 3, 1)
    ...
    return -torch.sum(log_sum_exp(log_probs), dim=[1,2]) # Just sum over all other dimensions, not the batch dimension
